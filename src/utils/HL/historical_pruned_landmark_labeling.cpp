...
void historical_pruned_landmark_labeling::construct_index(const vector<std::tuple<int, int, int>>& es) {
  // Setup the graph
  V = 0;
  for (const auto &e : es) {
...
#pragma omp parallel for schedule(guided, 1)
      rep (que_i, crr_que_size) {
		...
		_mm_prefetch((const char*)adj[v].data(), _MM_HINT_T0);
        ...
        // Label
		pdiff_labels.push_back(std::make_pair(v, label_entry_t{source_i, d, t }));
        ...
        rep (adj_i, adj[v].size()) {
          ...
          if (tt < crr_time[tv] && tt < nxt_time[tv]) {
            for (;;) {
              ...
              if (emulated_sync_bool_compare_and_swap(&nxt_time[tv], prv_tt, tt)) {
                if (prv_tt == INT_MAX) {
...
void historical_pruned_landmark_labeling::get_root_order(vector<int> &ord) {
  ...
  rep (v, V) deg[v] = std::make_pair(std::make_pair(adj[v].size(), rand()), v);
...
void historical_pruned_landmark_labeling::query_change_points(int v, int w, vector<pair<int, int>> &cp) {
	cp.clear();
