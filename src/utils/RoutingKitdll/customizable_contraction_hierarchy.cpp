...
namespace RoutingKit {
		...
		order(std::move(arg_order))
	{
		...
		if (log_message) {
			PROGRESS_BEGIN_MESSAGE("Building CCH \n");
		...
		if (log_message) {
			PROGRESS_BEGIN_MESSAGE("Start reordering nodes according to order \n");
		...
		if (log_message) {
			timer += get_micro_time();
			PROGRESS_BEGIN_MESSAGE("Finished reordering nodes, needed " + std::to_string(timer) + "musec \n");
		}
			...
			unsigned upper_treewidth_bound = compute_chordal_supergraph(
				...
				[&](unsigned x, unsigned y) {
					if (up_tail.size() == invalid_id) {
						if (log_message)
							PROGRESS_BEGIN_MESSAGE("CCH Construction aborted because chordal supergraph contains 2^32 or more arcs \n");
		...
		if (log_message) {
			...
			PROGRESS_BEGIN_MESSAGE("Chordal supergraph contains " + std::to_string(cch_arc_count) + " arcsc \n");
		}
		...
		if (log_message) {
			...
			PROGRESS_BEGIN_MESSAGE("The average number of nodes in a search space is " + std::to_string(nodes_in_search_space_sum / node_count) +"\n" );
			PROGRESS_BEGIN_MESSAGE("The maximum number of nodes in a search space is " + std::to_string(max_nodes_in_search_space) + "\n");
			PROGRESS_BEGIN_MESSAGE("The average number of arcs in a search space is " + std::to_string(arcs_in_search_space_sum / node_count) + "\n");
			PROGRESS_BEGIN_MESSAGE("The maximum number of arcs in a search space is " + std::to_string(max_arcs_in_search_space) + "\n");
		}
